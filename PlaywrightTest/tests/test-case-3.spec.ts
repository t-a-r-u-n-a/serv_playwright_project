...
test('should search TV, add to cart, and proceed to checkout', async ({ page }) => {
  ...
  await cartPage.incrementQuantity(2);
  const expectedCartTotal = await cartPage.getSubtotal();
